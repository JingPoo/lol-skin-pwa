...
<template>
    <div class="nav-container bg-primary text-white sticky top-0 z-50 md:fixed transition ease-in duration-300" :class="{'md:-translate-x-full': !navShow}">
        <nav class="h-10 w-full md:h-screen md:w-28 text-md flex flex-row md:flex-col justify-start items-center shadow relative">
            <!-- toggle button -->
            ...
            <ul class="w-[calc(100%-2rem)] md:w-full md:h-full md:mt-12 flex flex-row justify-center items-center md:flex-col md:justify-start text-md">
                <li v-for="role in roles" :key="role.id" class="w-full h-full md:h-max md:pb-4 leading-8 group relative text-center">
                    <a href="javascript:;" class="block h-full px-2 cursor-default hover:text-complementary hover:font-bold after:content-[''] after:border-b-2 after:border-complementary after:w-0 hover:after:w-full after:block after:transition-width after:duration-300">{{ role.text }}</a>

                    <!-- second nav -->
                    <ul class="bg-secondary z-max w-max absolute top-full left-2 md:-top-1 md:left-full rounded shadow md:shadow-2xl">
                        <li class="h-max md:h-10 px-2 border-2 rounded border-primary hidden group-hover:flex group-hover:flex-col group-hover:items-center group-hover:md:flex-row ">
                            <router-link 
                            class="w-max px-2 hover:text-complementary hover:font-bold hover:before:content-['']"
